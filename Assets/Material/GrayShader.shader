Shader "UI/CircleMask"
{
    Properties
    {
        ...
        [Toggle(UNITY_UI_ALPHACLIP)] _CircleToggle ("Circle Toggle", Range(0,1)) = 1
        [Toggle(UNITY_UI_ALPHACLIP)] _GrayToggle ("Gray Toggle", Range(0,1)) = 1
    }
    ...
    SubShader
    {
        ...
        CGPROGRAM
            ...
            float onlySmoothDelta(float2 texcoord) {
                
                float x = (2 * (texcoord.x - 0.5)) / _RadiusX;
                float y = (2 * (texcoord.y - 0.5)) / _RadiusY;
                float boundary = 0.5;
                if(texcoord.x < boundary) x = 1 - texcoord.x/boundary;
                if(texcoord.y < boundary) y = 1 - texcoord.y/boundary;
            ...
            fixed4 frag(v2f IN) : SV_Target
            {
                ...
                bool circleToggle = _CircleToggle == 1;
                if (circleToggle) {
                    color.a *= circleDelta(IN.texcoord);
                }else{
                    color.a *= onlySmoothDelta(IN.texcoord);
                }

                bool grayToggle = _GrayToggle == 1;
                if(grayToggle){
